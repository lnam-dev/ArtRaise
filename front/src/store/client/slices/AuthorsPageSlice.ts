...
export const authorsPageSlice = createSlice({
    ...
    reducers: {
        setSelectedLetterIndex: (state, action: PayloadAction<number>) => {
            ...
            state.mapLetterAuthor = getMapOfLettersAuthors(authors,isSelectedLetter ? [state.availableLetters[action.payload]] : state.availableLetters );
            },
        ...
        setAuthors: (state, action: PayloadAction<TAuthor[]>) => {
            ...
            const updatedAvailableLetters = getFirstLettersOfAuthorsName([...action.payload]);
            state.availableLetters = updatedAvailableLetters;
            state.mapLetterAuthor = getMapOfLettersAuthors([...action.payload], updatedAvailableLetters);
        }

    }
})
export const {setSelectedLetterIndex,setAvailableLetters,setAuthors} = authorsPageSlice.actions;
