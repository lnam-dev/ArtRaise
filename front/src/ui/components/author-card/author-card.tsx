...
const AuthorCard: FC<Props> = ({author}) => {
    ...
    return (
        <figure className={"flex flex-shrink flex-col w-full h-full bg-gray-950 break-inside-avoid"} onClick={() => {redirect(`/en/authors/${author.id}`)}}>
            <div
                className="relative block w-full text-white/90 font-namu"> {/* Aspect ratio for consistent image size */}
                <img
                ...
                <div className={"flex flex-row justify-center items-center cursor-pointer"}>
                    <div className={"px-6 py-4 text-5"}>
                        ...
                        <p className={"text-4 font-light"}>{`${artpieces?.length ?? 0} роботи`}</p>
                    </div>
