...
const CategoryPage = ({ artPieces, categories, pagination }: TCategoryPage) => {
	return (
		<main className="container mx-auto mt-14 lg:mt-18 xl:mt-[5rem] mobile-spacing">
			...
			<section className="columns-1 gap-6 sm:columns-2 lg:columns-3 xl:columns-4 space-y-12 mb-12">
				{artPieces.map((obj) => (
					<CardPurchase key={obj.id} card={obj} />
				))}
			</section>
			{pagination && <PaginationContainer pagination={pagination} />}
		</main>
	);
};
