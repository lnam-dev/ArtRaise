...
const FilterMapper: React.FC<Props> = ({className}) => {
    ...
    for (const key of filterKeys) {
        if (filterState[key].trim() !== "") {
            filterItems.push({
                value: filterState[key],
                onDelete: () => dispatch(removeFilter({filterKey: key}))
            })
        }
    }
    return (
        <div className={`w-full flex flex-row min-h-10 overflow-x-auto gap-5 flex-nowrap pb-2 mb-2 ${className}`}>
            {filterItems.map((value, index) =>
                <DefaultTag key={index} className={"border-none flex flex-row w-fit items-center stroke-gray-950 gap-3 cursor-default"}>
