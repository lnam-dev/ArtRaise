...
const Home = ({params}: { params: Promise<{ id: string }> }) => {
    ...
    const [selectedInfo, setSelectedInfo] = useState<SelectedInfo>(SelectedInfo.AuthorArtPieces)
    useEffect(() => {
        async function fetchData(id: string) {
            ...
            artpieces = artpieces.filter(piece => piece.author.id === authorData.id);
        ...
        <div className={"h-fit font-fixel px-4"}>
            <div className={"font-namu mt-20 grid grid-cols-4 gap-x-[1rem] w-full h-fit auto-rows-auto"}>
                ...
                <div className={"flex md:col-span-3 h-full max-h-full"}>
                    <img src={tempPhotoArdSrc} alt={"art photo"}
                         className={"object-cover w-full h-full md:col-span-3 max-h-[60vh]"}></img>
                </div>
            </div>

            {/*mobile*/}
            <div className={`grid grid-cols-4 gap-x-[1rem] w-full h-fit mt-20 md:hidden`}>
            ...
            {/*all*/}
            <SegmentTitle className={"mt-20"} link={{to:"/en/authors",name:"Всі автори"}}>Схожі автори</SegmentTitle>
            <div className="columns-1 gap-6 sm:columns-2 lg:columns-3 xl:columns-4 space-y-12 mt-10">
                {familiarArtists.map((artist, index) => <AuthorCard author={artist} key={index}/>)}
            </div>
            <CallToActionSection/>
        </div>
