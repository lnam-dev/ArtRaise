...
import { useAllArtPieces } from '~/ui/hooks/useAllArtPieces';
...
async function getAllArtPieces(): Promise<TArtPiece[]> {
  try {
    const response = await fetch(`${process.env.API_URL}artpieces/`);
    if (!response.ok) {
      throw new Error(`Failed to fetch all art pieces: ${response.status}`);
    }
